  ...
  return (
    ...
    >
      <div className="w-full">
        <div className="flex md:items-center justify-between gap-2 md:gap-8 my-2 md:my-4 flex-col md:flex-row">
          ...
          <div className="flex items-center gap-2 lg:gap-4">
            ...
            >
              ...
              >
                <span>Visit</span>
                <ArrowTopRightIcon className="ml-1 w-6 h-6 group-hover:opacity-100 transition-all" />
              </Button>
            </Link>
          </div>
        </div>
        <div className="mt-2 space-x-2">
          {[...(project.additionalTags ?? [])]?.map((tag) => (
            <Badge
              ...
              variant="outline"
              className={cn(
                bagdeClass[tag as keyof typeof bagdeClass] ?? 'bg-[#D3FFF7]',
                'flex-shrink-0',
              )}
            >
        ...
        <p className="text-para-sm tracking-tight font-thin mt-4 text-primary/80 line-clamp-3">
          {project.description}
        </p>
      </div>
